...
const Daily = () => {
    ...
    const [formattedData] = useMemo(() => {
        ...
        // console.log("dailyData within Daily: ", dailyData)
        ...
        Object.values(dailyData).reduce(
            (acc, { date, totalSales, totalUnits }) => {
                ...
                if (dateFormatted >= startDate && dateFormatted <= endDate) {
                    const splitDate = date.substring(date.indexOf('-') + 1).split('-');
                    totalSalesLine.data = [
                        ...totalSalesLine.data,
                        {
                            // x: splitDate,
                            // x: toLocaleDateString("en-US"),
                            x: `${dateFormatted.getMonth() + 1}/${dateFormatted.getDate()}/${dateFormatted.getFullYear()}`, // Format manually as MM/DD/YYYY
                            y: totalSales,
                        },
                    ];
                    totalUnitsLine.data = [
